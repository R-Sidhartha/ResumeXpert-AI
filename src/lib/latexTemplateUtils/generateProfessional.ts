...
export function generateProfessional(
  ...
  const placeholders: { [key: string]: string } = {
    ...
    "<<SUMMARY>>": resumeData.summary
      ? ` {\\large \\textbf{\\begin{tcolorbox}\\textsc{Professional Summary}\\end{tcolorbox}}} ${resumeData.summary} \\vspace{${resumeData.customization?.sectionSpacing || "0pt"}}
  `
      : ``,
    "<<EDUCATION>>": resumeData.educations?.length
      ? `{\\large \\textbf{\\begin{tcolorbox}\\textsc{Academic Qualifications}\\end{tcolorbox}}} \\begin{center} \\vspace{-4pt} \\begin{tabular}{|p{3.5cm}|p{6.0cm}|p{9.5cm}${hasScoreColumn ? "|p{2.5cm}" : ""}|} \\hline \\centering \\textbf{Year} & \\centering \\textbf{Degree/Certificate} & \\centering \\textbf{Institute}${hasScoreColumn ? ` & \\centering \\textbf{CPI/\\%}` : ""} \\tabularnewline
...
\\centering ${formatYear(edu.startDate)} -- ${formatYear(edu.endDate)} & \\centering ${highlightAndEscapeLatex(edu.degree)} & \\centering ${highlightAndEscapeLatex(edu.school)}${
    ...
    "<<SKILLS>>": resumeData.skills?.length
      ? `{\\large \\textbf{\\begin{tcolorbox}\\textsc{Technical Skills}\\end{tcolorbox}}} \\vspace{-5pt} \\begin{itemize} ${resumeData.skills
          .map(
            (skillGroup) =>
